 ...
 const handleExecuteCompletedStories = (pageId: string, selectedStoryIds?: string[]) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex overflow-hidden font-sans">
      ...
      <ColorsModal isOpen={isColorsModalOpen} onClose={() => setIsColorsModalOpen(false)} projectId={currentProject?.id} onSave={async (colorTheme: string) => { if (currentProject) await updateProject(currentProject.id, { colorTheme }); }} />
