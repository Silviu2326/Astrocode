...
export function ProjectProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (response.page) {
        ...
        const newPage: AppPage = {
          id: response.page.id,
          title: response.page.name || response.page.title,
          description: response.page.description,
          status: pageData.status || 'todo',
          priority: pageData.priority,
          createdAt: new Date(),
          updatedAt: new Date(),
          userStories: response.page.userStories || [],
        };
      ...
      // Si falla, crear página localmente como fallback
      const newPage: AppPage = {
        ...pageData,
        id: Date.now().toString(),
        createdAt: new Date(),
  ...
  const updatePage = async (projectId: string, pageId: string, pageData: Partial<AppPage>) => {
    try {
      // Hacer la petición PUT al backend
      await projectService.updatePage(projectId, pageId, pageData);
  ...
  const updateUserStory = (projectId: string, pageId: string, userStoryId: string, userStoryData: Partial<UserStory>) => {
    // Optimización: Solo actualizar el estado una vez y evitar re-renders innecesarios
    const updateFunction = (project: any) => {
      ...
      return {
        ...
        pages: project.pages.map((page: any) => {
          ...
          return {
            ...page,
            userStories: (page.userStories || []).map((story: UserStory) => 
    ...
    // Actualizar projects
    setProjects(prev => prev.map(updateFunction));

    // Solo actualizar currentProject si es necesario
    if (currentProject?.id === projectId) {
      setCurrentProject(prev => prev ? updateFunction(prev) : null);
    }
  };
